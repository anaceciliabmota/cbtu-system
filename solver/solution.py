...
class Solution:
    ...
    def get_solution(self, data, total_time: float = 0.0) -> dict:
        ...
        for t in range(data.nb_trains):
            ...
            for i in range(data.max_trips_per_train[t]):
                for r in range(data.nb_routes):
                    if not data.is_valid_route(t, i, r):
                        continue
                    if self.lambda_values[t][i][r] <= 0:
                        continue
                    trip_counter += 1
                    arcs = data.route_arcs[r]
                    if not arcs:
                        stops: list[dict] = []
                    else:
                        verts = [arcs[0]["out"]] + [a["inc"] for a in arcs]
                        stops = [
                            {
                                "point": int(data.vertex_to_point[v]),
                                "arrival": int(self.y_bar_values[t][i][v]),
                                "departure": int(self.y_values[t][i][v]),
                            }
                            for v in verts
                        ]
                    trips_out.append({"id": trip_counter, "stops": stops})
            trains_out.append({"id": t + 1, "trips": trips_out})

        return {
            "total_time": total_time,
            "solution_value": round(self.obj_value),
            "trains": trains_out,
        }
